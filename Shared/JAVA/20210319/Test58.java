...
public class Test58 {
	public static void main(String[] args) {
		...
		//x변수에 값은 변하지 않는다.
		//그이유는 기본자료형  변수 x,y는 각각 독립된 변수메모리이기 때문이다.
		System.out.println("기본자료형 변수값 출력");
		System.out.println("x->" + x + ", y->" + y);
			
		//---------참조자료형 변수값을 변경--------------------
		//참조변수 t로 new MyDate()객체의 각 객체변수에 접근 하여 값 변경
		t.year = 2007;
		t.month = 7;
		t.day = 9;
		//참조변수 d로 new MyDate()객체의 각 객체 변수에 접근 하여  출력하고
		//참조변수 t로 new MyDate()객체의 각 객체 변수에 접근 하여  변경한 값을 출력하면
		//t로 변경한 값들이 모두 출력 됨
		//이유 : 참조변수 d와 참조변수t는 하나의 new MyDate()객체를 동시에 참조 하고 있기 때문이다.
		System.out.println("참조자료형의 변수에 저장된 객체내부의 객체변수값들 출력");
		System.out.println(d.year + "/" + d.month + "/" + d.day );
		System.out.println(t.year + "/" + t.month + "/" + t.day );
