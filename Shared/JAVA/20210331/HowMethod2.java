...
class PerimeterMath1 { // 직사각형의 둘레계산클래스
	...
	public static double calRectanglePeri(double width, double height) {
		...
		// (가로 X 2) + (세로 X 2) <--풀어 쓰면
		// 직사각형 둘레 리턴
		return SimpleMath1.add( SimpleMath1.mul(width,2) , SimpleMath1.mul(height, 2) );
...
public class HowMethod2 {
	...
	//참고!! 원넓이 구하는 클래스메소드(static 메소드) 호출 , 호출시 2.4전달
	double result = AreaMath1.calCircleArea(2.4);
	System.out.println("원의 넓이 : " + result);	
			
	//참고!! 직사각형 둘레 구하는 클래스메소드(static 메소드) 호출, 호출시  2.0, 4.0 전달
	double result1 = PerimeterMath1.calRectanglePeri(2.0, 4.0);
	System.out.println("직사각형 둘레 : " + result1);
		// 출력 결과
		// 원의 넓이 : 18.09504
		// 직사각형 둘레 : 12.0
	}
...
/*
 	결론:
 	-> static 메소드를 선언하면 원넓이 계산 클래스나  둘레 계싼 클래스 내부에서
 	   SimpleMath1객체를 생성 하지 않고  SimpleMath1클래스명.클래스메소드 바로 호출이 가능 하다.
 	   장점 : 코드길이가 더 깔끔해 지고 간결해 진다.
