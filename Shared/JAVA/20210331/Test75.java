...
class InitTest{
	...
	//멤버변수(인스턴스변수)선언
	//-초기화 하지 않아도 자동으로 int 변수에는 기본값으로 0으로 초기화가 이루어짐
	int x; //0
...
public class Test75 {
	...
	static {
			for(int i=0;  i<arr.length; i++) {
				//1과 10사이의 랜덤값(int정수)을 얻어서 arr배열의 각인덱스 위치에 반복해서 저장
...
/*
	결론
	[배열에 값을 초기화할경우에는]
	명시적초기화 만으로는 복잡한 초기화 작업을 할수 없기때문에
	이런 경우에는 추가적으로 클래스 초기화블럭을 사용하여 배열에 값을 초기화 해야 한다.
