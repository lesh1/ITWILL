...
2. 자바의 메소드 종류 2가지
   ...
   <자바API문서 참고>
   - String클래스 내부에  toString()메소드  :  특정 데이터를 문자열로 변환 하는 기능의 메소드
   - Math클래스 내부에  random()메소드 : 0.0 ~ 1.0 랜덤값을 제공해 주는 기능의 메소드
   - Date클래스 내부에  getDate()메소드 : 현재 날짜 및 시간관련 데이터를 제공해 주는 기능의 메소드
   
   (2) 사용자 정의 메소드  : 우리 개발자가 특정 기능을 필요로 할때마다 새롭게 기능을 추가해서 만드는 메소드
    	1. 매개변수가 없고 반환할 값이 없는 메소드를 만들수 있다.
    	2. 매개변수가 있고 반환할 값이 없는 메소드를 만들수 있다.
    	3. 매개변수와 반환할 값이 있는 메소드를 만들수 있다.
...
public class Test47 {
	...
	//-> 메소드의 이름은 main이다.
	//-> 프로그램을 시작시키는 기능.
	//-> main메소드 내부에서 다른 메소드를 호출해서 사용할수 있다.
	//-> 같은 클래스 내부의 다른 메소드 호출시  클래스명.메소드명  <--문법을 사용한다. 
	...
	public static void main(String[] args) {
		...
		//out이라는 상수에 저장된 PrintStream객체 에 접근 하여 
		//PrintStream객체 내부에 .... println메소드가 만들어져 있으므로 메소드를 호출하여
		//그 기능을 사용 할수 있었다.
		System.out.println("안녕");
		//println(String value)메소드의 기능 : 매개변수 value로 전달 받은 데이터 출력  
