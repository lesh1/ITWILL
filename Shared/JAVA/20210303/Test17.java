...
public class Test17 {
	public static void main(String[] args) {
		byte a = 10;
		byte b = 20;
	//	byte c =  a + b; //<---수정 후  이유 주석쓰기 
		byte c =  (byte)(a+b);
		//이유 : a변수값과 b변수값은 byte타입이므로
		// 이항 연산인 덧셈연산을 하기전에  int타입으로 자동형변환된다.
		// int 10과  int 20을 덧셈 연산 하므로  연산결과는 int 값이 된다.
		//int타입의 값(4byte)을 byte타입(1byte) 변수에 담아야 하므로 형변환(byte)을 해주어야한다.
	...
	//	ch = ch + 2; // <---수정 후  이유 주석쓰기
		...
		//      char타입의 변수 ch에 저장하기 위해서는 (char)타입으로 형변환 해주어야 합니다.
		
		
		
	//	float f = 3 / 2;  //<---수정 후 이유 주석쓰기 
		float f = 3 / 2f;
		//이유 : int 와 int의 연산결과는 int이기 때문에 3 / 2의 결과는 1이 된다.
		// 연산결과를 실수값으로 얻고 싶으면,  적어도 두 피연산자 중 한쪽이 실수타입(float와 double중의 하나)
		// 이어야 한다.
	...
	//	long l  = 3000 * 3000  * 3000;// <---수정 후 이유 주석쓰기 
		...
		//이유 : int  * int * int의 결과는 int타입의 최대값인 약 2*10의 9제곱을 넘는 결과는 
