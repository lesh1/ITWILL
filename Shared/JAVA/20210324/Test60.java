/*
	...
	- 생성자의 역할: 클래스로 부터 객체 생성시 객체의 변수에 저장되는 상태값을 초기화할 목적.
	- 생성자 만들때 조건
	  1. 클래스의 이름과 동일한 이름으로 생성자이름을 정한다.
	  2. 리턴타입 X ,  값을 리턴 하지 않는다.
...
class Number{
	...
	//생성자2
	//- 하나의 값을 매개변수로 전달 받아 num변수에 저장 시키는 생성자 
	public Number(int n) {
		num = n;
...
public class Test60 {
	...
	public static void main(String[] args) {
		...
		//Number클래스로 부터 객체 생성시 Number(int n)생성자를 호출할때 30을 전달하여
		//num변수의 값을 초기화 시킴
...
//결론 : 객체 생성시 생성자를 호출하면...
//1.객체를 생성하는 동시에..인스턴스 변수의 값을 초기화 할수 있다.(한줄로 코드를 작성할수 있다)
//2.단 한번만 호출되는 생성자이니..final로 선언된 인스턴스변수의 초기화에도 사용이 가능하다.
